...
def parser(csvfile):
	...
	def getDonor(donor_name_f, email_f, want_receipt_f, telephone_number_f, mobile_number_f, address_line_f, city_f, province_f, postal_code_f, customer_ref_f):
        ...
        want_receipt_f = True if (want_receipt_f.lower() == 'email') or (
            want_receipt_f.lower() == 'e-mail') else False

        result_donor = None
        try:
            result_donor = Donor.objects.get(donor_name=donor_name_f, email=email_f, want_receipt=want_receipt_f, telephone_number=telephone_number_f, mobile_number=mobile_number_f,
                                             address_line=address_line_f, city=city_f, province=province_f, postal_code=postal_code_f, customer_ref=customer_ref_f, verified=True)
        except Donor.DoesNotExist:
            result_donor = Donor.objects.create(donor_name=donor_name_f, email=email_f, want_receipt=want_receipt_f, telephone_number=telephone_number_f, mobile_number=mobile_number_f,
                                                address_line=address_line_f, city=city_f, province=province_f, postal_code=postal_code_f, customer_ref=customer_ref_f, verified=True)
    ...
    def getDonation(donor_id_f, tax_receipt_no_f, donate_date_f, pick_up_f):
        ...
        try:
            result_donation = Donation.objects.get(
                tax_receipt_no=tax_receipt_no_f)
            ...
            result_donation = Donation.objects.create(
                donor_id=donor_id_f, tax_receipt_no=tax_receipt_no_f, donate_date=donate_date_f, verified=True, pick_up=pick_up_f)
	...
	def addItem(donation_id_f, description_f, particulars_f, manufacturer_f, model_f, quantity_f, working_f, condition_f, quality_f, batch_f, value_f):
		...
		Item.objects.create(tax_receipt_no=donation_id_f, description=description_f, particulars=particulars_f, manufacturer=manufacturer_f, model=model_f,
    ...
    for row in read_file:
        ...
        if(rowcount != 0):
            ...
            donor_id = getDonor(donor_name_f, email_f, want_receipt_f, telephone_number_f,
                                mobile_number_f, address_line_f, city_f, province_f, postal_code_f, customer_ref_f)
            # donation_id = tax_receipt_no
            donation_id = getDonation(
                donor_id, tax_receipt_no_f, donate_date_f, pick_up_f)
            addItem(donation_id, description_f, particulars_f, manufacturer_f, model_f,
