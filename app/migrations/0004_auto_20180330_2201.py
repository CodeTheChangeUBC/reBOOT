...
def update_created_at_formatted(apps, schema_editor):
    Donor = apps.get_model('app', 'Donor')
    for donor in Donor.objects.all():
        donor.created_at_formatted = '%s' % (donor.created_at.strftime("%Y-%m-%d"))
        donor.save()

    Donation = apps.get_model('app', 'Donation')
    for donation in Donation.objects.all():
        donation.created_at_formatted = '%s' % (donation.created_at.strftime("%Y-%m-%d"))
        donation.save()

    Item = apps.get_model('app', 'Item')
    for item in Item.objects.all():
        item.created_at_formatted = '%s' % (item.created_at.strftime("%Y-%m-%d"))
        item.save()
...
class Migration(migrations.Migration):
    ...
    operations = [
        ...
        migrations.RunPython(update_created_at_formatted),
    ]
